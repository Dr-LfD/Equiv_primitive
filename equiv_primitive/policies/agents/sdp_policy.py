...
from equiv_primitive.policies.vision.sdp_encoder import SDPEncoder
from equiv_primitive.policies.utils.diffusion.ema_model import EMAModel
from equiv_primitive.policies.utils.sdp_diffusion.irreps_conditional_unet1d import IrrepConditionalUnet1D
from equiv_primitive.policies.utils.normalizer import LinearNormalizer

from equiv_primitive.policies.utils.lan_utils import MLPEncoder
...
class SDPPolicy(nn.Module):
    """Predict end-effector trajectories from point clouds and text embeddings."""
    ...
    def _setup_language_encoder(self, network_name, **language_encoder_kwargs):
        if network_name != "MLPEncoder":
            raise ValueError(f"Unsupported language encoder: {network_name}")
        return MLPEncoder(**language_encoder_kwargs)
