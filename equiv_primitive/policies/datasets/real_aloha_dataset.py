...
class RealAlohaDataset(Dataset):
    ...
    def process_real_aloha_traj(self, cfg, **kwargs):
        ...
        cache_dir = os.path.join(EQUIV_PRIMITIVE_PATH, cfg.embedding_cache_dir) if hasattr(cfg, 'embedding_cache_dir') else None
                ...
                for obj_name in obj_names:
                    ...
                    for _ in range(aug_traj_nums):
                        for primitive_kw in primitive_kws:
                            # Process grasp skill
                            if primitive_kw == 'grasp':
                                if not len(grasp_poses):
                                    continue
                            ...
                            elif primitive_kw == 'place':
                                if not has_release or not len(release_poses):
                                    continue
                                ...
                                raise ValueError(f'Unsupported primitive: {primitive_kw}')
